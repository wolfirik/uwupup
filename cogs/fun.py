...
class Fun_Commands:
    ...
    async def lick(self, ctx, user: discord.Member=None):
        ...
        if not user:
            ...
            try:
                r = requests.get('https://e926.net/post/index.json?limit=1&tags=-kiss%20order:random%20face_lick%20-equine%20duo') 
                r = r.json()
                link = r[0].get('file_url')
                thing = BytesIO(await http.get(link, res_method="read"))
            except:
    ...
    async def kiss(self, ctx, user: discord.Member=None):
        ...
        if not user:
            ...
            lick = discord.Embed(description=f"**{ctx.author.name} kissed {user.name} ❤w❤**", color=0xf26522)
            lick.set_image(url=link)
            try:
                await ctx.send(embed=lick)
            except discord.Forbidden:
