    ...
    async def dbl_info(self, ctx, bot: discord.Member):
        ...
        if not bot.bot:
            ...
            if b.get("error") == "Not found":
                ...
                if bot.name.endswith("'s"):
                    name = bot.name
                elif bot.name.endswith("'"):
                    name = bot.name + "s"
                else:
                    name = bot.name + "'s"
    ...
    async def betawidget(self, ctx, bot: discord.Member, embed: bool=True):
        if not bot.bot:
            ...
            if b.get("error") == "Not found":
                ...
                link = f"http://dblw.glitch.me/widget?name={botto}&server_count={guilds}&votes={points}&owner={owner_name}&status={statuss}&avatar={bot.id}|{bot.avatar}&certified={cert}"
                if name.endswith("'s"):
                    name = name
                elif name.endswith("'"):
                    name = name + "s"
